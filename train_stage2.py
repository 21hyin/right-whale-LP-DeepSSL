...
def main():
    ...
    if args.isMT:
    	resume_fn = 'models/%s_%d_mean_teacher_split_%d_isL2_%d/checkpoint.best.ckpt' % (args.dataset,args.num_labeled,args.label_split,int(args.isL2))
    else:
    	resume_fn = 'models/%s_%d_split_%d_isL2_%d/checkpoint.best.ckpt' % (args.dataset,args.num_labeled,args.label_split,int(args.isL2))
